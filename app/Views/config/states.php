...
<div class="flex items-center justify-between pb-3 border-b border-gray-200 dark:border-gray-700 mb-6">
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-gray-900 dark:text-white text-xl font-bold leading-tight">State List</h1>
    </div>
...
<div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
    ...
    <table class="w-full">
        <thead class="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-10">
            <tr>
                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">State Name</th>
        ...
        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <?php if (!empty($states)): ?>
                <?php foreach ($states as $state): ?>
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/50">
                        <td class="px-4 py-3 text-sm font-medium text-gray-900 dark:text-white"><?= esc($state['name']) ?></td>
                        <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400"><?= esc($state['country_name'] ?? '-') ?></td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-2">
                                <button onclick="editState(<?= $state['id'] ?>, '<?= esc($state['name']) ?>', <?= $state['country_id'] ?>)" class="text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300">
                                    <span class="material-symbols-outlined text-lg">edit</span>
                                </button>
                                <form action="<?= base_url('config/states/toggle/' . $state['id']) ?>" method="POST" class="inline">
                                    <?= csrf_field() ?>
                                    <button type="submit" class="text-gray-600 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300">
                                        <span class="material-symbols-outlined text-lg"><?= $state['is_active'] ? 'toggle_on' : 'toggle_off' ?></span>
                                    </button>
                                </form>
                                <button onclick="confirmDelete('<?= base_url('config/states/delete/' . $state['id']) ?>', '<?= esc($state['name']) ?>')" class="text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300">
            ...
            <?php else: ?>
                <tr>
                    <td colspan="4" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">No states found</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
...
<!-- Add/Edit Modal -->
<div id="stateModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
    <div class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md mx-4">
        <h2 id="modalTitle" class="text-xl font-bold text-gray-900 dark:text-white mb-4">Add State</h2>
        <form id="stateForm" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="stateName" name="name" required placeholder="Insert state name..."
            ...
            <div class="mb-4">
                ...
                <select id="stateCountry" name="country_id" required
                        class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent">
...
<div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
    <div class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md mx-4 shadow-xl">
        ...
        <p class="text-gray-600 dark:text-gray-300 mb-6" id="deleteMessage">Are you sure you want to delete this state?</p>
        <div class="flex gap-3">
...
function openAddModal() {
    ...
    document.getElementById('stateForm').action = '<?= base_url('config/states/store') ?>';
...
function editState(id, name, countryId) {
    ...
    document.getElementById('stateForm').action = '<?= base_url('config/states/update') ?>';
    document.getElementById('stateId').value = id;
    document.getElementById('stateName').value = name;
    document.getElementById('stateCountry').value = countryId;
    document.getElementById('stateModal').classList.remove('hidden');
}
