...
<div class="flex items-center justify-between pb-3 border-b border-gray-200 dark:border-gray-700 mb-6">
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-gray-900 dark:text-white text-xl font-bold leading-tight">City List</h1>
    </div>
...
<div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
    ...
    <table class="w-full">
        <thead class="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-10">
            <tr>
                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">City Name</th>
                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">State</th>
                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Country</th>
        ...
        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <?php if (!empty($cities)): ?>
                <?php foreach ($cities as $city): ?>
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/50">
                        <td class="px-4 py-3 text-sm font-medium text-gray-900 dark:text-white"><?= esc($city['name']) ?></td>
                        <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400"><?= esc($city['state_name'] ?? '-') ?></td>
                        <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400"><?= esc($city['country_name'] ?? '-') ?></td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-2">
                                <button onclick="editCity(<?= $city['id'] ?>, '<?= esc($city['name']) ?>', <?= $city['state_id'] ?>, <?= $city['country_id'] ?>)" class="text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300">
                                    <span class="material-symbols-outlined text-lg">edit</span>
                                </button>
                                <form action="<?= base_url('config/cities/toggle/' . $city['id']) ?>" method="POST" class="inline">
                                    <?= csrf_field() ?>
                                    <button type="submit" class="text-gray-600 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300">
                                        <span class="material-symbols-outlined text-lg"><?= $city['is_active'] ? 'toggle_on' : 'toggle_off' ?></span>
                                    </button>
                                </form>
                                <button onclick="confirmDelete('<?= base_url('config/cities/delete/' . $city['id']) ?>', '<?= esc($city['name']) ?>')" class="text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300">
            ...
            <?php else: ?>
                <tr>
                    <td colspan="5" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">No cities found</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
...
<!-- Add/Edit Modal -->
<div id="cityModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
    <div class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md mx-4">
        <h2 id="modalTitle" class="text-xl font-bold text-gray-900 dark:text-white mb-4">Add City</h2>
        <form id="cityForm" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="cityName" name="name" required placeholder="Insert city name..."
            ...
            <div class="mb-4">
                ...
                <select id="cityCountry" name="country_id" required onchange="filterStates()"
                        class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent">
...
<div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
    <div class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md mx-4 shadow-xl">
        ...
        <p class="text-gray-600 dark:text-gray-300 mb-6" id="deleteMessage">Are you sure you want to delete this city?</p>
        <div class="flex gap-3">
...
function openAddModal() {
    ...
    document.getElementById('cityForm').action = '<?= base_url('config/cities/store') ?>';
...
function editCity(id, name, stateId, countryId) {
    ...
    document.getElementById('cityForm').action = '<?= base_url('config/cities/update') ?>';
...
function filterStates() {
    ...
    const allOptions = stateSelect.querySelectorAll('option[data-country]');
    ...
    // Reset state selection if current doesn't match country
