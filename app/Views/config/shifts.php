...
<div class="flex items-center justify-between pb-3 border-b border-gray-200 dark:border-gray-700 mb-6">
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-gray-900 dark:text-white text-xl font-bold leading-tight">List of Shift</h1>
    </div>
    ...
    <div class="overflow-x-auto max-h-[calc(100vh-0px)] overflow-y-auto">
        <table class="w-full">
            <thead class="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-10">
                <tr>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Shift Name</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                <?php if (!empty($shifts)): ?>
                    <?php foreach ($shifts as $shift): ?>
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/50">
                            <td class="px-4 py-3 text-sm font-medium text-gray-900 dark:text-white"><?= esc($shift['name']) ?></td>
                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400"><?= date('h:i A', strtotime($shift['start_time'])) ?></td>
                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400"><?= date('h:i A', strtotime($shift['end_time'])) ?></td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-2">
                                    <button onclick='editShift(<?= json_encode($shift) ?>)' class="text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300">
                                        <span class="material-symbols-outlined text-lg">edit</span>
                                    </button>
                                    <form action="<?= base_url('config/shifts/toggle/' . $shift['id']) ?>" method="POST" class="inline">
                                        <?= csrf_field() ?>
                                        <button type="submit" class="text-gray-600 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300">
                                            <span class="material-symbols-outlined text-lg"><?= $shift['is_active'] ? 'toggle_on' : 'toggle_off' ?></span>
                                        </button>
                                    </form>
                                    <button onclick="confirmDelete('<?= base_url('config/shifts/delete/' . $shift['id']) ?>', '<?= esc($shift['name']) ?>')" class="text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300">
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="5" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">No shifts found</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
...
<!-- Add/Edit Modal -->
<div id="shiftModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
    <div class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md mx-4">
        <h2 id="modalTitle" class="text-xl font-bold text-gray-900 dark:text-white mb-4">Shift Details</h2>
        <form id="shiftForm" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="shiftName" name="name" required placeholder="Enter shift name..."
            ...
            <div class="mb-4">
                ...
                <input type="time" id="startTime" name="start_time" required step="1"
            ...
            <div class="mb-4">
                ...
                <input type="time" id="endTime" name="end_time" required step="1"
                       class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-primary focus:border-primary [&::-webkit-calendar-picker-indicator]:dark:invert [&::-webkit-calendar-picker-indicator]:cursor-pointer">
            </div>
...
<div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
    <div class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md mx-4 shadow-xl">
        ...
        <p class="text-gray-600 dark:text-gray-300 mb-6" id="deleteMessage">Are you sure you want to delete this shift?</p>
        <div class="flex gap-3">
...
function openAddModal() {
    document.getElementById('modalTitle').textContent = 'Shift Details';
    document.getElementById('shiftForm').action = '<?= base_url('config/shifts/store') ?>';
...
function editShift(shift) {
    document.getElementById('modalTitle').textContent = 'Shift Details';
    document.getElementById('shiftForm').action = '<?= base_url('config/shifts/update') ?>';
