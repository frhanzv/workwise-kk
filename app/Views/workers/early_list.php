...
<div class="flex flex-col gap-6 max-w-[1600px] w-full mx-auto">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mt-6 md:mt-4">
        <div class="flex flex-col gap-1">
            <h1 class="text-gray-900 dark:text-white text-3xl font-bold tracking-tight">Staff Early List</h1>
            <p class="text-gray-500 dark:text-gray-400 text-base font-normal leading-normal">Workers who checked out early <?= esc($dateLabel) ?></p>
        </div>
    ...
    <div class="bg-white dark:bg-background-dark rounded-xl border border-gray-200 dark:border-gray-700 flex flex-col flex-1">
        ...
        <div class="overflow-x-auto">
            <table class="w-full" id="earlyStaffTable">
                ...
                <tbody class="bg-white dark:bg-background-dark divide-y divide-gray-200 dark:divide-gray-700">
                    <?php if (empty($earlyStaff)): ?>
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center gap-2">
                                    <span class="material-symbols-outlined text-5xl text-gray-300 dark:text-gray-600">event_available</span>
                                    <p class="text-gray-500 dark:text-gray-400 text-base font-medium">No early checkouts <?= $dateLabel === 'Today' ? 'today' : 'on this day' ?></p>
                                    <p class="text-gray-400 dark:text-gray-500 text-sm">All workers completed their shifts</p>
                                </div>
                            </td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($earlyStaff as $index => $staff): ?>
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors staff-row">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-xs text-gray-500 dark:text-gray-400"><?= esc($staff['shift']) ?> Shift (<?= esc($staff['shift_end']) ?>)</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="text-sm font-semibold text-red-600 dark:text-red-400"><?= esc($staff['check_out_time']) ?></span>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
...
<div id="detailModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white dark:bg-background-dark rounded-xl shadow-xl max-w-2xl w-full">
        ...
        <div class="p-6 space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Early Checkout Reason:</label>
                <textarea id="modalEarlyReason" rows="3" class="w-full px-4 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white rounded-lg text-sm resize-none"></textarea>
            </div>
        </div>
...
<script>
    ...
    function filterByDate(period) {
        window.location.href = '<?= base_url('workers/early-list') ?>?filter=' + period;
    }
    ...
    function filterByCustomDate() {
        ...
        if (dateInput.value) {
            window.location.href = '<?= base_url('workers/early-list') ?>?date=' + dateInput.value;
        }
    }
    ...
    function openDetailModal(staff) {
        ...
        document.getElementById('modalTimeIn').value = ''; // Empty for now
        document.getElementById('modalTimeOut').value = staff.check_out_time;
        document.getElementById('modalEarlyReason').value = ''; // Can be filled by user
    ...
    function approveStaff() {
        ...
        const earlyReason = document.getElementById('modalEarlyReason').value;
        
        // Here you would send an AJAX request to approve the staff member
        fetch('<?= base_url('workers/approve-early') ?>', {
            method: 'POST',
            headers: {
    ...
    function exportToCSV() {
        ...
        let exportUrl = '<?= base_url('workers/export-early-list') ?>?';
