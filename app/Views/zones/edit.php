<?= view('templates/header', ['title' => 'Edit Zone']) ?>
    ...
    <div class="flex flex-col gap-6 w-full">
        ...
        <div class="flex items-center gap-4">
            ...
            <div class="flex flex-col gap-2">
                <h1 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Edit Zone</h1>
                <p class="text-gray-500 dark:text-gray-400">Update zone configuration and antenna settings.</p>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
            <form action="<?= base_url('zones/update/' . $zone['zone_id']) ?>" 
                  method="post" 
                ...
                <div class="p-6 flex flex-col gap-8">
                    ...
                    <div class="flex flex-col gap-4">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-xs font-medium px-2 py-1 rounded bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">
                                <?= ucfirst($zone['status']) ?>
                            </span>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="flex flex-col gap-2">
                                ...
                                <input class="rounded-lg border-gray-300 dark:border-gray-600 bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 w-full p-2.5" 
                                       id="zone_id" 
                                       type="text" 
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <input class="rounded-lg border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white shadow-sm focus:border-primary focus:ring-primary w-full p-2.5" 
                                       id="zone_name" 
                                       name="zone_name" 
                                       type="text" 
                                       value="<?= esc($zone['zone_name']) ?>" 
                                       required/>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <textarea class="rounded-lg border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white shadow-sm focus:border-primary focus:ring-primary w-full p-2.5" 
                                      id="location" 
                                      name="location" 
                                      rows="2"><?= esc($zone['location'] ?? '') ?></textarea>
                        </div>

                        <!-- Current Location Picture -->
                        <?php if (!empty($zone['location_image'])): ?>
                        <div class="flex flex-col gap-2">
                            <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                                Current Location Picture
                            </label>
                            <div class="relative w-full max-w-md">
                                <img src="<?= base_url('uploads/zones/' . $zone['location_image']) ?>" 
                                     alt="Location" 
                        ...
                        <div class="flex flex-col gap-2">
                            <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                                <?= !empty($zone['location_image']) ? 'Replace Location Picture' : 'Upload Location Picture' ?>
                            </label>
                            <input type="file"
                                   name="location_image"
                            ...
                            <div class="antenna-block flex flex-col gap-4 p-5 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                                <div class="flex items-center justify-between mb-2">
                                    <h3 class="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">
                                        Antenna <?= str_pad($index + 1, 2, '0', STR_PAD_LEFT) ?>
                                    </h3>
                                    <?php if ($index > 0): ?>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                                    <div class="flex flex-col gap-2">
                                        ...
                                        <div class="relative">
                                            ...
                                            <input class="pl-10 p-2.5 rounded-lg border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white shadow-sm focus:border-primary focus:ring-primary w-full" 
                                                   name="antenna_ip[]" 
                                                   value="<?= esc($antenna['ip_address']) ?>" 
                                                   type="text" 
                                                   required/>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex flex-col gap-2">
                                        ...
                                        <input class="p-2.5 rounded-lg border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white shadow-sm focus:border-primary focus:ring-primary w-full" 
                                               name="antenna_port[]" 
                                               value="<?= esc($antenna['port']) ?>" 
                                               type="number" 
                                               min="1" 
                                               max="65535" 
                                               required/>
                                    ...
                                    <div class="flex flex-col gap-2">
                                        ...
                                        <select class="p-2.5 rounded-lg border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white shadow-sm focus:border-primary focus:ring-primary w-full" 
                                                name="antenna_mode[]" 
                                                required>
                                            <option value="">Select Antenna Mode</option>
                                            <?php foreach($antennaModes as $mode): ?>
                                                <option value="<?= esc($mode['mode_name']) ?>" <?= ($antenna['antenna_mode'] == $mode['mode_name']) ? 'selected' : '' ?>>
                                                    <?= esc($mode['mode_name']) ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
...
<script>
let antennaCount = <?= count($antennas) ?>;
